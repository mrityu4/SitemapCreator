  ...
  return (
    <div>
      <div
        ...
        className="absolute z-40 flex items-center p-1 px-3 bg-gray-600 -bottom-1 rounded-3xl"
      >
        <button
          key="duplicate"
          className="w-8 h-8"
          onClick={() => duplicateBlock(data.key)}
        >
        ...
        <button
          key="wireframe"
          className="w-8 h-8"
          onClick={() => handleFramePicker(true)}
        >
      ...
      {iscolorPickerOpen && (
        <div
          ref={colorPickerRef}
          className="absolute z-40 flex flex-wrap justify-start w-32 bg-gray-600 rounded-xl"
        >
          {colors.map((c) => (
            <button
              key={c}
      ...
      {isFramePickerOpen && (
        <div
          ref={framePickerRef}
          className="absolute z-40 flex flex-wrap justify-start w-48 bg-gray-600 rounded-xl"
        >
